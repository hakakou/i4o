    ...
    public static class IndexSetExtensions
    {
        public static IEnumerable<T> WhereUsingIndex<T>(IndexSet<T> indexSet, Expression<Func<T, bool>> predicate)
        {
            ...
            switch (binaryBody.NodeType)
            {
                ...
                default:
                    throw new Exception("No index for " + predicate.Name);
        ...
        public static T SingleIndexed<T>(this IndexSet<T> indexSet, Expression<Func<T, bool>> predicate)
        {
            return WhereUsingIndex(indexSet, predicate).Single();
        }
        public static T SingleOrDefaultIndexed<T>(this IndexSet<T> indexSet, Expression<Func<T, bool>> predicate)
        {
            return WhereUsingIndex(indexSet, predicate).SingleOrDefault();
        }
        public static T FirstIndexed<T>(this IndexSet<T> indexSet, Expression<Func<T, bool>> predicate)
        {
            return WhereUsingIndex(indexSet, predicate).First();
        }
