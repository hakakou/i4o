    ...
    public class PropertyReaderTests
    {
        ...
        private static IndexReadTimingResult GetIndexReadTimingResult(int numberOfItems)
        {
            var dummyCollectionOfData = (from x in Enumerable.Range(1, numberOfItems)
                                         select new TestClassA { TestValue = Guid.NewGuid().ToString() })
                .ToList();

            Func<PropertyReadStrategy, int, long>
                timeFor = (readerStrategy, iterations) =>
                 {
                     var stopwatch = Stopwatch.StartNew();
                     var reader = new PropertyReader<TestClassA>(readerStrategy, "TestValue");
